...
module.exports = class extends PostAPI {
    ...
    processLink(link, thumbnail) {
        const iscdo = link.match(
            /^https?:\/\/studio\.code\.org\/projects\/(applab|gamelab)\/([^/]+)/
        )
        const isscratch =
            link.match(/^https?:\/\/scratch\.mit\.edu\/projects\/(\d+)/) ||
            link.match(/^https?:\/\/turbowarp\.org\/(\d+)/)
        const iskhan = link.match(
            /^https?:\/\/www\.khanacademy\.org\/computer-programming\/([^/]+\/\d+)/
        )
        if (!thumbnail && iscdo)
            thumbnail = `https://studio.code.org/v3/files/${iscdo[2]}/.metadata/thumbnail.png`
        if (!thumbnail && isscratch)
            thumbnail = `https://uploads.scratch.mit.edu/get_image/project/${isscratch[1]}_432x288.png`
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            if (project.posterId !== user.id && user.role !== 'Admin')
    ...
    async favorite(req, res, next) {
        try {
            ...
            owner.notify(
                user.username + ' favorited your project!',
                project.name,
                '/project/' + project._id,
                user._id,
                user.username
