...
fs.writeFileSync(`${self.csvPath}/${table_name}.csv`, self.jsonToCSV(table.records), "utf-8");
  ...
  db.readRecords = function (table_name) {
    let table = this._data.tables[table_name];
    if (table === undefined) 
      throw `failed to read table ${table_name}`;
    return table.records;
  }
...
module.exports = {
  Database: function(a) {
    ...
    c(a,"post","populate_key_values", (db,req) => db.populateKeyValues(req.body.key_values_json));
    ...
    c(a,"get","read_records", (db,req) => db.readRecords(req.query.table_name));
