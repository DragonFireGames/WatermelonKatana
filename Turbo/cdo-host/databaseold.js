const fs = require("fs");
if(!fs.existsSync("./client/Turbo/Database/")){fs.mkdirSync("./client/Turbo/Database")}
...
class Database {
    ...
    constructor(app, path) {
        ...
        if (!fs.existsSync(this.directory)) { this.updateData() }
        else { this.getData() }
        if (fs.existsSync(this.csvPath)) {
            let csvDirs = fs.readdirSync(this.csvPath);
            for (let file of csvDirs) {
                let tablename = file.match(/.*(?=\.)/)[0];
                if (file.endsWith(".csv") && this.tableContainer[tablename] === undefined) {
                    this.tableContainer[tablename] = this.csvToJSON(fs.readFileSync(`${self.csvPath}/${file}`, "utf-8"));
                } else if (file.endsWith(".json")) {
                    let container = this.keyContainer;
                    container = Object.assign(this.keyContainer, JSON.parse(fs.readFileSync(`${self.csvPath}/${file}`, "utf-8")))
                }
            }
        } else {
        ...
        if (!initalized) {
            ...
            app.put(genURL("populate_key_values"), (req, res) => {
                ...
                key_values_json = typeof key_values_json === "string" ? JSON.parse(key_values_json) : key_values_json;
            ...
            app.post(genURL("create_record"), (req, res) => {
                ...
                if (typeof table_name !== "string" || (typeof record_json !== "object" && record_json !== null)) {
                    return self.error(res, `invalid argument table "${table_name}" or record "${record_json}"`)
                }
                self.getData();
            ...
            app.post(genURL("create_table"), (req, res) => {
                ...
                if (typeof table_name !== "string") { return self.error(res, `unable to create a table wihout a name`) }
                self.getData();
                let table = self.tableContainer;
                if (table[table_name] === undefined) { table[table_name] = self.emptyTable }
                self.updateData();
                res.send(true);
            })
            app.post(genURL("add_column"), (req, res) => {
                let { column_name, table_name } = req.body;
                if (typeof column_name !== "string" || typeof table_name !== "string") { return self.error(res, `invalid argument table "${table_name}" or column "${column_name}"`) }
                self.getData();
                let table = self.tableContainer[table_name];
                if (table === undefined) { return self.error(res, `unable to create a column without a table`) }
            ...
            app.post(genURL("add_shared_table"), (req, res) => {
                ...
                if (!fs.existsSync(file)) {
                    self.getData();
                    self.tableContainer[table_name] = self.csvToJSON(fs.readFileSync(file, "utf-8"))
                    res.send(true);
                }
            })
            app.post(genURL("import_csv"), (req, res) => {
                let { table_name, table_data_csv } = req.body;
                if (typeof table_name !== "string" || typeof table_data_csv !== "string") { return self.error(res, `unable to import csv table ${table_name} with data ${table_data_csv}`) }
                self.getData();
                let json = self.csvToJSON(table_data_csv)
                if (json.records.length < 0) { return self.error(res, "there isn't any data in this csv file") }
            ...
            app.put(genURL("populate_tables"), (req, res) => {
                ...
                if (typeof tables_json !== "string") { return self.error(res, `invalid params json "${json}"`) }
            ...
            app.put(genURL("update_record"), (req, res) => {
                ...
                record_json = JSON.parse(record_json);
                if (typeof table_name !== "string" || (typeof record_json !== "object" && record_json !== null)) {
                    return self.error(res, `unable to update table ${table_name} at id ${table_id}`)
                }
                self.getData();
                let table = self.tableContainer;
            ...
            app.put(genURL("rename_column"), (req, res) => {
                ...
                let table = self.tableContainer[table_name];
                if (typeof old_column_name !== "string" || typeof new_column_name !== "string" || table === undefined) {
                    return self.error(res, `invalid argument on table "${table_name}" column "${old_column_name}" new column "${new_column_name}"`)
                }
                for (let record of table.records) {
            ...
            app.put(genURL("coerce_column"), (req, res) => {
                ...
                let table = self.tableContainer[table_name];
                if (typeof column_name !== "string" || column_type.match(/string|number|boolean/)[0] === null || table === undefined) {
                    return self.error(res, `invalid argument on table "${table_name}" column "${column_name}" type "${column_type}"`)
                }
                for (let record of table.records) {
                    switch (column_type) {
                        case "boolean": {
            ...
            app.get(genURL("get_columns_for_table"), (req, res) => {
                ...
                let table = self.tableContainer[table_name];
                if (table === undefined) { self.error(res, `no table found at "${table_name}"`) }
                for (let record of table.records) {
                    for (let p in record) {
            ...
            app.get(genURL("export_csv"), (req, res) => {
                ...
                let { table_name } = req.query;
                let table = self.tableContainer[table_name];
                if (table === undefined) { return self.error(res, `table "${table_name}" cannot be exported`) }
            ...
            app.get(genURL("read_records"), (req, res) => {
                ...
                if (table === undefined) { return self.error(res, `failed to read table ${req.query.table_name}`) }
                res.send(table.records);
            })
            app.delete(genURL("clear_table"), (req, res) => {
                ...
                let { table_name } = req.body;
                let table = self.tableContainer
                if (table[table_name] === undefined) { return self.error(res, `failed to clear table "${table_name}"`) }
                table[table_name] = self.emptyTable;
                self.updateData();
                res.send(true);
            ...
            app.delete(genURL("delete_record"), (req, res) => {
                ...
                let table = self.tableContainer[table_name];
                if (table === undefined || table.records[record_id - 1] === undefined) { return self.error(res, `failed to remove record on table "${table_name}" at id "${record_id}"`) }
            ...
            app.delete(genURL("delete_column"), (req, res) => {
                ...
                let table = self.tableContainer[table_name];
                if (table === undefined) { return self.error(res, `failed to remove column on table "${table_name}" at column "${column_name}"`) }
                for (let record of table.records) {
            ...
            app.delete(genURL("delete_table"), (req, res) => {
                ...
                let { table_name } = req.body;
                let table = self.tableContainer;
                if (table[table_name] === undefined) { return self.error(res, `failed to delete the table "${table_name}"`) }
            ...
            app.get(genURL("project_has_data"), (req, res) => {
                self.getData();
                res.send(Object.keys(self.keyContainer).length > 0 || self.tableContainer.length > 0);
            })
    ...
    csvToJSON(csv) {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            for (let j = 0; j < headers.length; j++) {
                obj[headers[j]] = rowData[j] ? rowData[j].replace(/"/g, '') : null;
            }
